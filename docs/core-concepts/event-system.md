...
```cpp
void phaseCommit(UpdateContext& updateContext) {
    // 1. Clear pass event bus - pass events don't cross phases
    passEventBus_.clearAll();
    
    // 2. Flush commands
    gameWorld_.flushCommandBuffers(updateContext);
    
    // 3. Flush managers
    gameWorld_.flushManagers(updateContext);
    
    // 4. Swap phase event bus - events become readable in next phase
    // including events generated by the managers
...
for (phase : {Pre, Main, Post}) {
    ...
    gameWorld.flushCommandBuffers(updateContext); // Execute commands
    gameWorld.flushManagers(updateContext);        // Process manager queues
