...
Die Strukturierung der Engine folgt der im Prototyp-Dokument beschriebenen Unterteilung in Module und orientiert sich weitgehend an einem \textit{by-feature, by-layer}-Ansatz, wie sie in Abbildung~\ref{fig:engine_struktur} dargestellt ist.\\
...
\begin{itemize}
    ...
    Diese basieren auf Expression Builder, Fluent Interfaces sowie einer DSL, um die Konfiguration von Fachobjekten aus einem hohen Abstraktionsgrad in einen besser lesbaren Kontext zu überführen.
...
\begin{itemize}
    ...
    Als primär datenhaltende \texttt{struct}s konzipiert, können diese Objekte auch Invarianten absichern, etwa durch \textit{Clamping} von Werten, sollten darüber hinaus jedoch kein Verhalten implementieren.\footnote{
        Im Gegensatz zu unserem Konzept, das Invarianten erlaubt, sehen andere ECS-Architekturen wie \textit{Unity DOTS} vor, dass Komponenten als reine Informationsträger fungieren: Invarianten werden dort in den Systemen abgesichert~\cite{UnityComponentConcepts}.
    }
    ...
    \item \texttt{types}: Hilfstypen wie Kontextobjekte / \textit{Data Transfer Objects} zum Datentransport sowie typisierte IDs zur sicheren Identifizierung von Ressourcen zur Laufzeit.
\end{itemize}
