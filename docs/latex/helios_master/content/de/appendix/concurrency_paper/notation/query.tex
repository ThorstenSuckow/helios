...
\noindent
Analog ist der Beweis für mehr Spalten und daraus folgenden Zerlegungen zu führen, die der gleichen Form genügen.

\begin{theorem}\label{f_min_norm}
    Das Relationsschema $K$ erfüllt die Bedingungen der 1., 2., 3., BCNF-, 4., 5, -Normalform.
\end{theorem}

\begin{proof}
    Für den Nachweis greifen wir auf die minimale funktionale Abhängigkeitsmenge zurück
    ...
    Im Folgenden weisen wir der Reihe die 5 Normalformen sowie die BCNF schrittweise nach, und erfüllen damit implizit den Nachweis der Erfüllung aller NF $< n$ für eine NF $n, n \geq 1$\footnote{
        Für die BCNF gilt, dass sie in 3.NF ist; als verschärfte Bedigung der 3. NF gilt dies aber nicht im umgekehrten Fall~\cite[S. 328]{EN11}.
    }\\

    \noindent
    Um die Beweise einfacher zu gestalten, wiesen wir zunächst nach, dass sich $K$ nicht weiter zerlegen lässt, ohne, dass die Bedingung für einen verlustfreien JOIN erfüllt ist.\\


    \textbf{nicht} die Eigenschaft eines verlustfreien JOINs sowie keine mehrwertigen Attribute aufweist.\\
    Da $e$ der einzige Kandidatenschlüssel in den Relationen ist sowie alle anderen Attribute $A \in R, A \neq e$ nicht-prim sind,


    \noindent
    Außerdem zeigen wir, dass keine mehrwertige Abhängigkeit existiert:  Gäbe es eine solche Abhängigkeit, müsste $K$ mindestens zwei unabhängige 1:N Beziehungen A:C, A:D aufweisen, was aufgrund des Primärschlüssels $e$ als einziges Prime-Attribut nicht möglich ist.

    \medskip
    \begin{itemize}
        \item 1. Normalform: Erfüllt, da $K$ keine mehrwertigen Atribute enthält.
        \item 2. Normalform: Erfüllt, da $K$ als Kandiatenschlüssel nur $e$ enthält, und somit keine partiellen Abhängigkeiten existieren.
        \item 3. Normalform: Erfüllt, da $K$ keine transitiven Abhängigkeiten enthält, was anhand $F_\text{min}$ zu sehen ist.
        \item BCNF: Erfüllt, da für jede nicht-triviale funktionale Abhängigkeit $X \rightarrow A\ \in F_\text{min}$ $X$ der Kandidatenschlüssel ist.
        \item 4. Normalform: Erfüllt, da $K$ keine mehrwertigen Abhängigkeiten besitzt.
        \item 5.Normalform: Erfüllt, da $K$ mit $JD(K) \equiv *(\pi_{K}(r)) = r$ eine einzelne, triviale Join-Abhängigkeit aufweist. $K$ weist bereits eine verlustfreie Join-Zerlegung auf, und damit per Defintion in Project-Join-Normalform ist.
    \end{itemize}
\end{proof}
