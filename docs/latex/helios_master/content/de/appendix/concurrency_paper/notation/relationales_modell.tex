...
\[
\begin{alignedat}{3}
  \text{dom}(k_i) &= K_i + \{\unitval\}\\
  \text{dom}(k_j) &= K_j + \{\unitval\}\\
    \ldots
\end{alignedat}
\]
...
\begin{lemma}\label{lem:K_plus_ist_verlustfrei}
    Das Relationsschema $K^\top = K(e, k^\top_1, k^\top_2, \ldots, k^\top_n), n \in \mathbb{N}$ lässt sich verlustfrei in die Relationen $K^\top_1, K^\top_2, \ldots, K^\top_n$ zerlegen.
\end{lemma}
...
Eine Zerlegung
    ...
    \]
     heißt redundanzfrei, wenn für alle $R_i \in D$ gilt:
    \[
        (\bigcup_{i=1, i \neq j}^n R_i) \neq K^{\top}, \quad 1 \leq j \leq n
    \]
    \medskip
    Damit schließt ``redundanzfrei`` in diesem Kontext eine mehrfach vorkommende, gleiche Attributabdeckung aus, während nicht-disjunkte Teilmengenbeziehungen $R_i \cap R_j \neq \emptyset$ zulässig sind.
\end{definition}
\bigskip


\begin{lemma}\label{lem:redundanzfreier_kern}
    Jede Zerlegung
    \[
        D = (R_1, R_2, \ldots, R_n), \text{ mit } K^\top = R_1 \cup R_2 \cup \ldots \cup R_n
    \]
    lässt sich auf eine redundanzfreie Zerlegung $D'$ reduzieren.
\end{lemma}
\begin{proof}
    Sei $D$ eine beliebige Zerlegung von $K^\top$, sei $D' = D$.\\

    Falls $D'$ nicht redundanzfrei ist, existiert mindestens ein $1 \leq j \leq n$, so dass

    \[
        \bigcup_{i=1, i \neq j} R_i = K^\top
    \]
    Dieses $R_j$ wird aus $D'$ entfernt.
    Die Schritte werden so lange wiederholt, bis $D'$ redundanzfrei ist, und damit gilt
    \[
        (\bigcup_{i=1, i \neq j}^n R_i) \neq K^{\top}, \quad 1 \leq j \leq n
    \]
    Es ist sichergestellt, dass das Verfahren terminiert, da mit der Voraussetzung $D'$ eine Zerlegung von $K^\top$ ist.
    Ist die Bedingung der Redundanzfreiheit verletzt, kann aber auch mindestens ein redundantes Element entfernt, ohne dass
    \[
        (\bigcup_{i=1}^n R_i) = K^{\top}, R_i \in D'
    \]
    verletzt wird -- somit kann die Zerlegung auch nicht leer sein.
 end{proof}

\bigskip
Wir gehen im Folgenden implizit davon aus, dass jede Zerlegung im Sinne von~\ref{lem:redundanzfreier_kern} auch redundanzfrei ist.\\
...
\begin{definition}\label{def:zerlegung}
    Sei $n = \lvert K^\top \setminus \{e\} \rvert, n > 1$.
    $e$ ist der Primärschlüssel der Relation $K^\top(e, k^\top_1, k^\top_2, \ldots, k^\top_n)$.\\
    $k^\top_x, x \leq n$ ist Nicht-Prime-Attribut.\\

    \noindent
    Dann bezeichnet
    \[
        D^n_{m_l} = (R_1, \ldots, R_m),\quad 1 < m \leq \lvert K^\top \setminus \{e\} \rvert
    \]
    mit
    \[
        \begin{alignedat}{3}
    ...
    \]
    \smallskip
    eine Zerlegung von $K^\top$.
\end{definition}
\medskip
Das Subscript $l \in \mathbb{N}$ bezeichnet in Definition~\ref{def:zerlegung} einen Index, der eine der möglichen Zerlegungen in $m$ Relationen indiziert.
Eine Ordnungsrelation für $l$ bezüglich der indizierten Zerlegungen ist im weiteren Kontext nicht relevant.
\bigskip

\begin{lemma}\label{lem:zerlegung_bekommt_schluessel}
Jede Zerlegung von $K^\top$ kann in eine Zerlegung $D^n_{m_l}$ transformiert werden.
\end{lemma}
\begin{proof}
    Sei $n = \lvert K^\top \setminus \{e\} \rvert$.\\
    Sei $D = (R_1, R_2, \cup, R_m)$ eine beliebige redundanzfreie Zerlegung von $K^\top$, so dass $K^\top = R_1 \cup R_2 \cup \ldots \cup R_m$.
    Dann kann zu jeder Relation der Primärschlüssel von $K^\top$ hinzugefügt werden:
    \[
        \forall R_i \in D: R_i = R_i \cup \{e\}
    \]
    womit nach Definition
    \[
        D^n_{m_l} = (R_1, \ldots, R_m)
    \]
    gilt.
    Weitere Möglichkeiten einer Zerlegung $D^n_{m_1}, D^n_{m_2}, \ldost$ ergeben sich durch den gegenseitigen Austausch von Komponentenattributen unter den Relationen.
\end{proof}
\bigskip

\begin{example}
Die Zerlegungen $D^3_{2_l}$ beschreiben Zerlegungen der Relation $K^\top$ mit den drei Komponentenattributen $k^\top_1, k^\top_2, k^\top_3$. Jede Zerlegung beinhaltet $m=2$ Relationen:
\[
    \begin{alignedat}{3}
        D^3_{2_1} &= \{ \{e, k^\top_1\}, \{e, k^\top_2, k^\top_3 \} \}\\
        D^3_{2_2} &= \{ \{e, k^\top_2\}, \{e, k^\top_1, k^\top_3 \} \}\\
        D^3_{2_3} &= \{ \{e, k^\top_3\}, \{e, k^\top_1, k^\top_2 \} \}\\
        D^3_{2_4} &= \{ \{e, k^\top_1, k^\top_3\}, \{e, k^\top_1, k^\top_2 \} \}\\
        D^3_{2_5} &= \{ \{e, k^\top_2, k^\top_3\}, \{e, k^\top_1, k^\top_2 \} \}\\
        &\vdots
    \end{alignedat}
\]
\end{example}
...
\begin{lemma}\label{lem:k_abhaengigkeitserhalten}
    Jede redundanzfreie Zerlegung $D^n_{m_l}$ ist abhängigkeitserhaltend,
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lem:jede_K_zerlegung_ist_verlustfrei}
    Jede redundanzfreie Zerlegung $D^n_{m_l}$ auf Basis der funktionalen Abhängigkeiten $F_{K^\top}$ ist verlustfrei.
\end{lemma}
...
\begin{lemma}\label{lem:k_alle_zerlegungen_einer_klasse}
    Jede redundanzfreie, verlustfreie Zerlegung von $K^\top, \lvert K^\top \rvert > 2$ bezüglich $F_{K^\top}$ gehört zu der Klasse $D^n_{m_l}$.
\end{lemma}
\begin{proof}
    Zu zeigen ist:
    \[
        \forall R_i \in D: e \in R_i
    \]
    \medskip
    Da $D$ nach Voraussetzung eine Zerlegung von $K^\top$ ist, gilt nach Definition~\ref{def:zerlegung}:
    \[
        K^{\top} = D = R_1 \cup R_2 \cup \ldots \cup R_n
    \]
    \medskip
    Angenommen, $\exists R_i \in D: e \notin R_i$.
    Dann gibt es für das Chase-Verfahren mindestens eine Zeile, die in der $e$-Spalte kein $a$-Symbol enthält.
    Da $D$ redundanzfrei ist, existiert außerdem in dieser Zeile mindestens eine Komponentenspalte $k_i$, die nicht über eine andere Relation mit einem $a$-Symbol besetzt ist.\\
    Damit verbleibt nach der Anwendung des Chase-Verfahrens mindestens eine Spalte, die nicht mit $a$-Symbolen besetzt ist, und damit kann keine Zeile existieren, die vollständig mit $a$-Symbolen besetzt ist.
    Damit ist die Zerlegung nicht verlustlos, im Gegensatz zur Annahme.\\
    Also muss in jeder Relation auch das $e$-Attribut vorkommen, womit $D$ insgesamt zu der Klasse $D^n_{m_l}$ gehört.
\end{proof}
...
\begin{tcolorbox}[excursusbox, title={Definition der 5. Normalform in der Literatur}]
    ...
    In ähnlicher Form findet sich diese Definition allerdings auch bei Elmasri und Navathe:
    \begin{displayquote}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        Jede JD impliziert verlustfreie Relationen $R_1, R_2, \ldots, R_3$.
        Mit Lemma~\ref{lem:k_alle_zerlegungen_einer_klasse} haben wir gezeigt, dass jede verlustfreie Zerlegung von $K^\top$ zu der Klasse $D^n_{m_l}$ gehört.\\
        Mit Lemma~\ref{lem:k_alle_zerlegungen_einer_klasse} haben wir gezeigt, dass Zerlegungen der Klasse $D^n_{m_l}$ auch verlustfreie Zerlegung von $K^\top$ repräsentieren.\\
        \item
        Mit Lemma~\ref{lem:redundanzfreier_kern} haben wir gezeigt, dass zu jeder Zerlegung von $K^\top$ eine redundanzfreie Zerlegung konstruiert werden kann.\\
        Mit Lemma~\ref{lem:zerlegung_bekommt_schluessel} haben wir gezeigt, dass zu jeder redundanzfreien Zerlegung von $K^\top$ eine Zerlegung der klasse $D^n_{m_l}$ konstruiert werden kann.\\
        Mit Lemma~\ref{lem:k_alle_zerlegungen_einer_klasse} haben wir gezeigt, dass Zerlegungen der Klasse $D^n_{m_l}$ auch verlustfreie Zerlegung von $K^\top$ repräsentieren.\\
        Mit Korollar~\ref{cor:k_joinabhaengigkeiten} haben wir gezeigt, das eine jede solche Zerlegung auch eine Join-Abhängigkeit induziert, die mit der Definition $D^n_{m_l}$ aus Relationen $R_i$ besteht, für die gilt:
        \[
            \forall R_i \in D^n_{m_l}: e \in R_i
        \]
        Damit enthält eine jede solche Relation den Primärschlüssel $e$ mit $e^+ = K^\top$.
