...
Die folgenden Lemma nutzen die Notation $D^n_{m_l}$ für eine redundanzfreie Zerlegung von $K^\top$ mit $n$ Komponentenattributen und $m$ Relationen:
\medskip
...
\begin{definition}\label{def:zerlegung}
    ...
    \]
    \smallskip
    eine redundanzfreie Zerlegung von $K^\top$.
\end{definition}
\medskip
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item 5. Normalform: Zu zeigen ist, dass jede JD von $K^\top$ allein von den Schlüsselabhängigkeiten von $R$ impliziert wird.\\
        Mit Lemma~\ref{lem:k_alle_zerlegungen_einer_klasse} haben wir gezeigt, dass jede redundanzfreie, verlustfreie Zerlegung von $K^\top$ zu der Klasse $D^n_{m_l}$ gehört.
