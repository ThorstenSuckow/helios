...
Das Subscript $l$ bezeichnet in Definition~\ref{def:zerlegung} einen Index, der eine der möglichen Zerlegungen in $m$ Relationen indiziert.
Eine Ordnungsrelation für $l$ ist in unserem Kontext nicht relevant.\\
\medskip
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item 5.Normalform: Zu zeigen ist, dass für jede nichttriviale Join-Abhängigkeit
        \[
            \text{JD}(R_1, R_2, \ldots, R_m)
        \]
        bezüglich $(F_{K^\top})^+$ jedes $R_x$ ein Superschlüssel für $K^{\top}$ ist.\\
        Mit Korrolar~\ref{cor:k_joinabhaengigkeiten} haben wir gezeigt, dass jede Zerlegung $D^n_{m_l}$ eine Join-Abhängigkeit bezüglich $K^\top$ induziert.\\
        Jede Zerlegung dieser Art ist nichttrivial, da mit Definition~\ref{def:zerlegung} gilt:
        \[
            \forall R_x \in D^n_{m_l}: R_x \neq K^{\top}
        \]
        Die FDs der Relationen jeder Zerlegung sind von der Form
        \[
            F_{R_x} = \{e  \rightarrow k^\top_x \mid k^\top_x \in R_x \setminus \{e\}\}
        \]
        wobei $e$ der Primärschlüssel von $K^\top$ ist -- damit ist jedes $R_x$ auch ein Superschlüssel für $K^\top$.
        Der Superschlüssel stellt außerdem als identifizierendes Attribut sicher, dass in jeder von $(F_{K^\top})^+$ abgeleiteten Zerlegung, die eine nichttriviale JD aufweist, jede Relation dieser Zerlegung auch ein Superschlüssel von $K^\top$ ist.
    \end{itemize}
\end{proof}
