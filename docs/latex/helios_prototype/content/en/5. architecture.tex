...
\begin{figure}[!h]
    ...
    \caption{Division of the architecture of the \textit{helios} framework into \textit{hard} and \textit{soft architecture} according to Rollings and Morris~\cite[pp.~612]{RM04}.
...
In the sense of \textit{Evans}, this is crucial for high cohesion: the directory names communicate the functionalities they contain~\cite[pp.~180]{Eva03}.
...
\textit{helios} also supports the dynamic addition of application controllers~\cite[379]{Fow03} to define isolated, event-based control logic, such as behavior during a window resize.\footnote{
...
Interested observers can register callbacks with the \texttt{EventManager} using \texttt{subscribe()}~\cite[pp.~293]{GHJV94}.
