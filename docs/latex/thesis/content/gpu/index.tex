\chapter{Typsichere GPU-Datenflüsse in einer OpenGL-basierten ECS-Engine: Normalisierte Komponentenrelationen, domänenspezifische Handles und heuristische Instanzierung}

\section{Forschungsfrage}

\begin{quote}
    Wie kann ein normalisiertes, komponentenzentrisches ECS-Datenmodell eine typsichere, ECS-domänenbezogene Beschreibung von GPU-Passes in einer OpenGL-basierten Engine motivieren, und welche Resource-Dependencies sowie Batching-Entscheidungen lassen sich daraus systematisch ableiten?
\end{quote}

\section{Methodik}

Die Arbeit untersucht, wie ein normalisiertes, komponentenzentrisches ECS-Datenmodell als Grundlage einer typsicheren Beschreibung von GPU-Datenflüssen in \texttt{helios} dienen kann.
...
Auf dieser Grundlage wird untersucht, wie die in der ECS-Schicht verwendeten domänenspezifischen Handles auf die Rendering-Schicht übertragen werden können, so dass Pass-Inputs und Pass-Outputs typseitig beschrieben werden und Resource-Dependencies während der Pass-Graph-Erzeugung systematisch aus diesen Signaturen abgeleitet werden können.\\
...
Aufbauend auf der normalisierten Zerlegung werden GPU-Passes über typisierte Lese- und Schreibsignaturen beschrieben.
...
Die konkrete Zuordnung auf GPU-Ressourcen, etwa auf OpenGL-Buffer-Objekte, erfolgt anschließend über die Resource-Registry beziehungsweise den Pass-Graph der Engine.
Der Zugriff bezieht sich damit auf die durch \texttt{ParticleHandle} beschriebenen Domäne der GPU-seitigen Buffer.\\
...
Wie die zugehörigen Ressourcen physisch repräsentiert werden -- etwa durch In-place-Updates oder Double-Buffer-Strategien mit unterschiedlichen physischen Buffern für Lese- und Schreibzugriff -- ist Gegenstand der Untersuchung.\\

Als konkreter Anwendungsfall der typsicheren Pass-Beschreibung wird instanziertes Rendering untersucht.
...
\begin{enumerate}
    ...
    \item \textbf{Typsichere Pass-Beschreibung und Resource-Dependencies:} GPU-Passes werden über typisierte Lese-/Schreibsignaturen mit ECS-Domänenbezug deklariert.
    Aus den deklarierten Signaturen wird ein Pass-Graph aufgebaut, dessen Resource-Dependencies durch die typisierten Signaturen vorbereitet und während der Graph-Erzeugung systematisch abgeleitet werden.
...
\begin{enumerate}
    ...
    \item Anzahl der Buffer-Wechsel und Updates der Instance-Buffer pro Frame,
    ...
    \item Anzahl der aus den typisierten Lese-/Schreibsignaturen ableitbaren Pass-Abhängigkeiten,
    \item Messung der Code Komplexität anhand statischer Code-Analyse-Tools.
\end{enumerate}
...
\section{Abgrenzung zum Referenzszenario}

Das Referenzszenario dieser Arbeit legt den Fokus auf ein Partikelsystem mit hoher Objektanzahl, sowie einer Szene mit mehreren statischen und dynamischen Objekten, die unterschiedliche Material-, Shader- und Mesh-Konfigurationen aufweisen.
Diese Konfiguration soll sicherstellen, dass klar instanzierbare Objektgruppen -- wie etwa Partikel mit gleicher Mesh- und Shader-Konfiguration -- als auch heterogene Objektmengen bereitgestellt werden, an denen sich der Einfluss des Schwellwerts \(\theta\) beobachten lässt.\\
...
Die CPU verwaltet in diesem Fall nur den initialen Partikelzustand; daneben zugehörige GPU-Ressourcen, deren Rollen innerhalb der Pass-Ausführung sowie globale Parameter.
Dadurch kann untersucht werden, welche Daten in der CPU-seitigen ECS-Struktur verbleiben und welche Daten als GPU-geführte Buffer-Repräsentation modelliert werden können, und wie eine sinnvolle Abbildung von komponentenzentrischen CPU-Daten auf die GPU erfolgen kann:
Dabei wird untersucht, inwieweit sich aus den \texti{Sparse-Set}-basierten Komponentenrelationen und Ressourcen-Handles Instancing-Gruppen ableiten lassen.
...
\begin{enumerate}
    ...
    \item Typsichere Beschreibung von GPU-Passes mit domänenspezifischen Handles
    \item Pass-Graph und Resource-Dependencies aus typisierten Signaturen
