/**
 * @file MotionBuilder.ixx
 * @brief Builder for motion-related component configuration.
 */
module;

export module helios.gameplay.builder.gameObject.builders.MotionBuilder;

import helios.gameplay.builder.gameObject.builders.configs.Move2DConfig;
import helios.gameplay.builder.gameObject.builders.configs.SteeringConfig;

export namespace helios::gameplay::builder::gameObject::builders {

    /**
     * @brief Builder for configuring movement and steering components.
     *
     * Provides factory methods for obtaining config objects that
     * set up 2D movement and rotation behavior on a GameObject.
     */
    template<typename Entity>
    class MotionBuilder {

        /**
         * @brief Non-owning pointer to the target GameObject.
         */
        Entity gameObject_;

    public:

        /**
         * @brief Constructs a MotionBuilder for the given GameObject.
         *
         * @param gameObject Target GameObject to configure.
         */
        explicit MotionBuilder(Entity gameObject) : gameObject_(gameObject) {}

        /**
         * @brief Returns a config for 2D movement setup.
         *
         * @return Move2DConfig for fluent configuration.
         */
        helios::gameplay::builder::gameObject::builders::configs::Move2DConfig<Entity> move2D() {
            return helios::gameplay::builder::gameObject::builders::configs::Move2DConfig<Entity>{gameObject_};
        }

        /**
         * @brief Returns a config for steering/rotation setup.
         *
         * @return SteeringConfig for fluent configuration.
         */
        helios::gameplay::builder::gameObject::builders::configs::SteeringConfig<Entity> steering() const {
            return helios::gameplay::builder::gameObject::builders::configs::SteeringConfig<Entity>{gameObject_};
        }

    };

}
