# helios::gameplay::effects

Visual effects module for game entities.

## Overview

This module provides components and systems for visual effects that enhance the presentation layer without affecting game logic.

## Submodules

| Submodule | Purpose |
|-----------|---------|
| `gfx/` | Graphics effects (spin, rotation animations) |

---

<details>
<summary>Doxygen</summary><p>
@namespace helios::gameplay::effects
@brief Visual effects module for game entities.
@details Provides visual effects components and systems for enhanced presentation.
</p></details>
